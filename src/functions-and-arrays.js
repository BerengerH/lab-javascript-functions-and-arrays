...
function findLongestWord(someArray) {
  ...
  for (i = 0; i < someArray.length; i++) {
    if (i !== 0) {
      if (someArray[i].length > someArray[i - 1].length && someArray[i].length > longestWord.length) {
        longestWord = someArray[i];
      } else if (someArray[i].length < someArray[i - 1].length && longestWord.length < someArray[i - 1].length) {
        longestWord = someArray[i - 1];
      }
    }
  }
  return longestWord;
...
function averageWordLength(someArray) {
  return sumContent(someArray) / someArray.length;
}
...
function uniquifyArray(someArray) {
  ...
  someArray.forEach(function (element, index) {
    if (!someArray.includes(element, index + 1) && !newArray.includes(element, index + 1)) {
...
function doesWordExist(someArray, word) {
  return someArray.includes(word);
}
